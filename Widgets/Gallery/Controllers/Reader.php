...
class Reader extends \Kotchasan\Controller
{
    ...
    public function get(Request $request)
    {
        ...
        if ($contents != '') {
            ...
            $listcount = $rows * $cols;
            echo '<div class="widget-gallery document-list col'.$cols.' small2 thumbview">';
            for ($i = 0; $i < sizeof($rss) && $listcount > 0; $i++) {
                $img = empty($rss[$i]['media:thumbnail']['url']) ? $rss[$i]['enclosure']['url'] : $rss[$i]['media:thumbnail']['url'];
                $text = htmlspecialchars($rss[$i]['title']['data']);
                $url = htmlspecialchars($rss[$i]['link']['data']);
                echo '<div class=item>';
                echo '<a href="'.$url.'" title="'.$text.'">';
                echo '<figure style="background-image:url('.$img.')"></figure>';
